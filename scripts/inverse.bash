...
if [ ! $# -eq 4 ]; then
    echo "./inverse.bash [32 or 128] [separate or joint] [kernel type] \
[training data]"
    exit 1
fi
...
if [ ${MODEL_TYPE} == "separate" ]; then
    MODEL_DIR="results/inverse-${KL}/2M-${KERNEL_TYPE}/${N}"
    if [ ! -d ${MODEL_DIR} ]; then
        mkdir -p ${MODEL_DIR}
    fi
    ...
    MODEL_DIR="results/inverse-${KL}/JM-${KERNEL_TYPE}/${N}"
    if [ ! -d ${MODEL_DIR} ]; then
        mkdir -p ${MODEL_DIR}
    fi
